...
This can be done with the [Diesel CLI tool](https://crates.io/crates/diesel_cli){:target="\_blank"}.
...
You'll also notice that this table macro doesn't use `rust` types in it's definition, for example `Text` and `Nullable`. We need to map these database types and constraints to a `struct` in rust to represent the data.
...
All going well, `cargo check` should now pass 🎉 time!
...
If our testcase relied on mock data, we would pass something like `MockDataInserts::none().user_accounts()` to get the required mock data in the system. This slows down the test case though, as it needs to insert lots of data before continuing.
...
By clicking into the `docs` on the left hand side, you should be able to see that the CreateUserAccountInput type now contains a nickname field!
...
You'll notice that even after updating the schema that even though we've updated the schema, we're still getting an error.
...
> For now you don't need to learn all about graphql queries, but you'll want to learn about them if you're not already familiar.
...
Our form looks great, but it does't work yet...
You'll notice that if you enter a nickname, click ok, then reopen the user edit form the nickname information is lost :(
...
This tutorial is mainly focused on the backend code, we won't try to explain the frontend code structure, but the in short, there's an api.ts file that is used when we sent requests to the backend, this need to be updated to send the nickname as part of the graphql request.
...
Now you should be able to update a create a user account with a nickname!
...
The tutorial didn't explicitly tell you how to do but you might need to add the code to `generate` in `backend/service/src/user_account/update.rs`
